...
def async_send_to(client, data, server_addr, server_port):
    try:
        # if it is socks5 proxy, will suspend the thread;
        addr = server_addr.decode("utf-8")
        client.sendto(data, (addr, server_port))
    except IOError as e:
...
class UDPRelay(object):
    def __init__(self, config, dns_resolver, is_local, stat_callback=None):
        self._config = config
        self.thread_pool = ThreadPoolExecutor(128)
        if is_local:
    ...
    # addr, port target ips，for choise dinstanse socks5 IP;
    def _create_remote_socket_for_socks5relay(self, addr, port):                            
        remote_sock = socks.socksocket(socket.AF_INET, socket.SOCK_DGRAM, socket.SOL_UDP)
        ...
        if remote_sock == None:
            logging.info("Error upd socks5 proxy error!")
            return None
        return remote_sock
